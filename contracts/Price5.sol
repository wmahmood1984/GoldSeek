 ...
 //   uint256 public DividendPerToken;
    ...
    function buy(address referrer)public payable returns(uint256){
        ...
        (
         ...
         uint256 drc,
         uint256 Idrc,
         uint256 afee) = getTValues(msg.value);
        
        
        uint256 tokens = ethereumToTokens_(tTransfer);
    address indRferrer = _referrerMapping[referrer];
        
        _holderBalances[msg.sender] += tokens; 
        ...
        else{_ReferralCommission[_admin]= drc; _ReferralCommission[_admin]= Idrc;}
    ...
    function sell(uint256 number) public payable {
         ...
         uint256 valueOFSell = number*rate / 100000000000;
        
        (
          uint256 tTransfer,uint256 tfee) = getSValues(valueOFSell);
    ...
    function processDiv() internal {
        rateOfDiv = DividendPool * magnitude / TotalSupply;
    }
